...
class StableDiffusionGuidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        # view_dependent_prompting: bool = True # 是否使用视角依赖的prompt
    ...
    def forward_unet(
        ...
        # print(latents.shape) torch.Size([8, 4, 64, 64])
        return self.unet(
    ...
    def compute_grad_sds(
        ...
        if prompt_utils.use_perp_neg: # false
            ...
            # print(self.cfg.view_dependent_prompting) # False
            # predict the noise residual with unet, NO grad!
            # print(latents.shape, t.shape, text_embeddings.shape) # torch.Size([4, 4, 64, 64]) torch.Size([4]) torch.Size([8, 77, 1024])
            with torch.no_grad():
                # add noise
                noise = torch.randn_like(latents)  # TODO: use torch generator
                latents_noisy = self.scheduler.add_noise(latents, noise, t)
                # pred noise
                latent_model_input = torch.cat([latents_noisy] * 2, dim=0) # 输入两倍的，无条件
                noise_pred = self.forward_unet(
                    latent_model_input,
                    torch.cat([t] * 2),
                    encoder_hidden_states=text_embeddings,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # print(rgb_BCHW.shape) # torch.Size([4, 3, 738, 994])
        latents: Float[Tensor, "B 4 64 64"]
        if rgb_as_latents:
            latents = F.interpolate(
        ...
        # print(latents.shape) # torch.Size([4, 4, 64, 64])
