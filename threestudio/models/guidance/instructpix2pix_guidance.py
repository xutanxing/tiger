...
class InstructPix2PixGuidance(BaseObject):
    ...
    class Config(BaseObject.Config):
        ...
        ddim_scheduler_name_or_path: str = "/home/project/instruct-nerf2nerf/ai/stable-diffusion-v1-4"
        ip2p_name_or_path: str = "/home/project/instruct-nerf2nerf/ai/instruct-pix2pix"
    ...
    def compute_grad_sds(
        ...
        image_cond_latents: Float[Tensor, "B 4 DH DW"],
        t: Int[Tensor, "B"],
    ):  #  torch.Size([12, 77, 768]) torch.Size([4, 4, 92, 124]) torch.Size([12, 4, 92, 124]) torch.Size([4])
        # print(text_embeddings.shape, latents.shape, image_cond_latents.shape, t.shape)
        with torch.no_grad():
            # add noise
            noise = torch.randn_like(latents)  # TODO: use torch generator
            latents_noisy = self.scheduler.add_noise(latents, noise, t)
            # pred noise
            latent_model_input = torch.cat([latents_noisy] * 3) # 没问题
            latent_model_input = torch.cat(
                [latent_model_input, image_cond_latents], dim=1
            )

            noise_pred = self.forward_unet(
                latent_model_input, torch.cat([t] * 3), encoder_hidden_states=text_embeddings
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        temp = torch.zeros(batch_size) # 4
        # print(batch_size)
        text_embeddings = prompt_utils.get_text_embeddings(temp, temp, temp, False)
        text_embeddings = torch.cat(
            [text_embeddings, text_embeddings[-batch_size:]], dim=0
        )  # [positive, negative, negative]

        # timestep ~ U(0.02, 0.98) to avoid very high/low noise level
        # print(self.min_step,self.max_step) # 20 200 0.02~0.2
        t = torch.randint(
            self.min_step,
            self.max_step + 1,
            [batch_size],
